...
const verifyUser = require("../middleware/verify.middleware.js");
const { verifyGoldUser } = require("../middleware/verify.premium.middleware.js");
...
router.get("/platinum/discount/:productId", async(req,res)=>{
    try{
        ...
        //both of the codes are not same..
        first one take more tinme
        and the second one is half..
        as inn the case of second it runns parallel
        //it takes array of asynchrnouous code...and it runns all of them parallelly 
        */
        const [user, product] = await Promise.all([
            await User.findById(userId),
            await Product.findById(productId)
